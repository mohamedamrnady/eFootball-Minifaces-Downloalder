...
local function make_key(ctx, filename)
    if string.match(filename, "common\\render\\symbol\\player") then
        ...
        local no_team_faces = get_files_in_dir(string.format("%s\\%s\\*", fileroot, player_id), "dds")
        if not tableIsEmpty(no_team_faces) then
            teams.insert("")
        end
        local index = has_a_or_b(teams, home, away) or math.random(1, #teams)
